...
// Fetch all short notes semester-wise
try {
    $query = "SELECT semester, subject, file_name FROM short_notes ORDER BY semester ASC";
    $result = $conn->query($query);
    if (!$result) {
        throw new Exception("Failed to fetch short notes.");
    }

    $upload_directory = "Uploads/short_notes"; // Directory where short notes are stored

    // Grouping short notes by semester
    $short_notes_by_semester = [];
    while ($row = $result->fetch_assoc()) {
        $short_notes_by_semester[$row['semester']][] = $row;
    }
} catch (Exception $e) {
    $error_message = "An error occurred while fetching short notes. Please try again later.";
    $short_notes_by_semester = [];
}
?>
...
<head>
    ...
    <meta name="description" content="View semester-wise short notes on AKTU Rankers, curated for B.Tech, M.Tech, and BCA students.">
    <meta name="keywords" content="AKTU Rankers, short notes, B.Tech short notes, M.Tech short notes, BCA short notes, semester short notes">
    <meta name="author" content="AKTU Rankers Team">
    <meta name="robots" content="index, follow">
    <meta property="og:title" content="View Short Notes - AKTU Rankers">
    <meta property="og:description" content="Access concise, semester-wise short notes tailored for AKTU students.">
    <meta property="og:type" content="website">
    <meta property="og:url" content="http://yourdomain.com/short_notes.php">
    <meta property="og:image" content="http://yourdomain.com/assets/og-image.jpg">
    <title>View Short Notes - AKTU Rankers</title>
    <link rel="canonical" href="http://yourdomain.com/short_notes.php" />
    ...
    <style>
        * {
        ...
        .short-note-item:hover {
            transform: translateY(-4px);
            box-shadow: 0 8px 24px rgba(0, 0, 0, 0.15);
        }

        .short-note-item::before {
            content: '';
            position: absolute;
            top: 0;
            left: 0;
...
<body>
    ...
    <main>
        <section class="short-notes-section">
            <h2>Semester-Wise Short Notes</h2>
            <?php if (isset($error_message)): ?>
                <p class="error-message"><?php echo htmlspecialchars($error_message); ?></p>
            <?php elseif (!empty($short_notes_by_semester)): ?>
                <div class="accordion">
                    <?php foreach ($short_notes_by_semester as $semester => $short_notes): ?>
                        <div class="accordion-item">
                            <div class="accordion-header">
                                Semester <?php echo htmlspecialchars($semester); ?>
                            </div>
                            <div class="accordion-content">
                                <?php foreach ($short_notes as $note): ?>
                                    <div class="short-note-item">
                                        <span class="short-note-subject"><?php echo htmlspecialchars($note['subject']); ?></span>
                                        <div class="short-note-actions">
                                            <button class="btn btn-view" onclick="openViewer('<?php echo htmlspecialchars($upload_directory . '/' . $note['file_name']); ?>')">View</button>
                                            <a href="<?php echo htmlspecialchars($upload_directory . '/' . $note['file_name']); ?>" class="btn btn-download" download>Download</a>
                                        </div>
                                    </div>
                                <?php endforeach; ?>
                            </div>
            ...
            <?php else: ?>
                <p class="no-content">No short notes available.</p>
            <?php endif; ?>
        </section>
    </main>
