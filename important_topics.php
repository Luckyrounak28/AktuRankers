...
// Fetch all important topics semester-wise
try {
    $query = "SELECT semester, subject, file_name FROM important_topics ORDER BY semester ASC";
    $result = $conn->query($query);
    if (!$result) {
        throw new Exception("Failed to fetch important topics.");
    }

    $upload_directory = "Uploads/important_topic"; // Directory where Important Topics PDFs are stored

    // Grouping Important Topics by semester
    $topics_by_semester = [];
    while ($row = $result->fetch_assoc()) {
        $topics_by_semester[$row['semester']][] = $row;
    }
} catch (Exception $e) {
    $error_message = "An error occurred while fetching important topics. Please try again later.";
    $topics_by_semester = [];
}
?>
...
<head>
    ...
    <meta name="description" content="View semester-wise important topics on AKTU Rankers, curated for B.Tech, M.Tech, and BCA students.">
    <meta name="keywords" content="AKTU Rankers, important topics, B.Tech important topics, M.Tech important topics, BCA important topics, semester important topics">
    <meta name="author" content="AKTU Rankers Team">
    <meta name="robots" content="index, follow">
    <meta property="og:title" content="View Important Topics - AKTU Rankers">
    <meta property="og:description" content="Access semester-wise important topics tailored for AKTU students.">
    <meta property="og:type" content="website">
    <meta property="og:url" content="http://yourdomain.com/important_topics.php">
    <meta property="og:image" content="http://yourdomain.com/assets/og-image.jpg">
    <title>Important Topics - AKTU Rankers</title>
    <link rel="canonical" href="http://yourdomain.com/important_topics.php" />
...
<body>
    ...
    <main>
        <section class="topics-section">
            <h2>Semester-Wise Important Topics</h2>
            <?php if (isset($error_message)): ?>
                <p class="error-message"><?php echo htmlspecialchars($error_message); ?></p>
            <?php elseif (!empty($topics_by_semester)): ?>
                <div class="accordion">
                    <?php foreach ($topics_by_semester as $semester => $topics): ?>
                            ...
                            <div class="accordion-content">
                                <?php foreach ($topics as $topic): ?>
                                    <div class="topic-item">
                                        <span class="topic-subject"><?php echo htmlspecialchars($topic['subject']); ?></span>
                                        <div class="topic-actions">
                                            <button class="btn btn-view" onclick="openViewer('<?php echo htmlspecialchars($upload_directory . '/' . $topic['file_name']); ?>')">View</button>
                                            <a href="<?php echo htmlspecialchars($upload_directory . '/' . $topic['file_name']); ?>" class="btn btn-download" download>Download</a>
                                        </div>
                                    </div>
                                <?php endforeach; ?>
                            </div>
            ...
            <?php else: ?>
                <p class="no-content">No important topics available.</p>
            <?php endif; ?>
        </section>
    </main>
